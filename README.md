...
$ npm run start
```
You can connect to this RPC server via `localhost:8545` with chain ID of 31337

### Deploying contracts to local fork of Mainnet

In order to successfully for the existing network, `ALCHEMY_API` must be set to a valid URL in the `.env` file.
```
...
$ npm run fork
```
You can connect to this RPC server via `localhost:8545` with chain ID of 1.
