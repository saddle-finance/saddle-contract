...
contract TestSwapReturnValues {
    ...
    function test_swap(uint8 tokenIndexFrom, uint8 tokenIndexTo, uint256 dx, uint256 minDy) public {
        ...
        require(returnValue == balanceAfter.sub(balanceBefore),
    ...
    function test_removeLiquidity(uint256 amount, uint256[] memory minAmounts) public {
        ...
        for (uint8 i = 0; i < n; i++) {
            ...
            require(balanceAfter[i].sub(balanceBefore[i]) == returnValue[i],
    ...
    function test_removeLiquidityImbalance(uint256[] calldata amounts, uint256 maxBurnAmount) public {
        ...
        require(returnValue == balanceBefore.sub(balanceAfter),
            "removeLiquidityImbalance()'s return value does not match burned lpToken amount");
    }

    function test_removeLiquidityOneToken(uint256 tokenAmount, uint8 tokenIndex, uint256 minAmount) public {
