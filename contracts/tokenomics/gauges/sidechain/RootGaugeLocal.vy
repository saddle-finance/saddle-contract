...
# needed for local testing, because RootGauge and 
# ChildChainStreamer need to have different  
# addresses on (single) testnet
child_chain_streamer: public(address)
...
def __init__(
    ...
    # commented out because for testing we send the SDL directly  
    # to ChildChainStreamer instead of the bridge
...
@external
def set_child_chain_streamer(addr: address):
    """
    @notice Set the address, to which the tokens are sent           
    @dev This is a helper method and will be removed in prod.
         Just needed because RootGaugeLocal and ChildChainStreamer
         can't have same address on single network
    @param addr Address of the ChildChainStreamer
    """
    assert addr != ZERO_ADDRESS
    
    self.child_chain_streamer = addr