...
describe("SimpleRewarder with MiniChefv2 but with ", async () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      // Don't transfer any reward tokens (mimicking Arbitrum setup)
  ...
  describe("onSaddleReward", () => {
    ...
    it("Reverts on calling harvest", async () => {
      // Set rewarder of pid 0 to the SimpleRewarder contract
      await miniChef.set(0, 1, simpleRewarder.address, true)
      // Try harvesting. We expect it to be reverted
      await expect(
        miniChef.connect(farmer).harvest(0, farmerAddress),
  ...
  describe("deposit", () => {
    ...
    it("Successfully claims secondary reward token when calling deposit", async () => {
      // Set rewarder of pid 0 to the SimpleRewarder contract
      await miniChef.set(0, 1, simpleRewarder.address, true)
      // Trigger an update by calling deposit with 0 amount
      await miniChef.connect(farmer).deposit(0, 0, farmerAddress)

      // Skip ahead 1000 seconds. We expect farmer to recieve about 2000e18 rewardToken2
  ...
  describe("withdrawAndHarvest", () => {
    ...
    it("Reverts when calling withdrawAndHarvest()", async () => {
      expect(await usdv2LpToken.balanceOf(farmerAddress)).to.eq(
        BIG_NUMBER_1E18.mul(5),
      )
      expect(await rewardToken1.balanceOf(farmerAddress)).to.eq(
  ...
  describe("emergencyWithdraw", () => {
    ...
    it("Successfully calls emergencyWithdraw and claims only rewardToken2", async () => {
      ...
      // Emergency withdraw will harvest only rewardToken2
      expect(await rewardToken2.balanceOf(farmerAddress)).to.eq(
        BIG_NUMBER_1E18.mul(2002),
      )
    })
