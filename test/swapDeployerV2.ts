...
describe("Swap DeployerV2", () => {
  let signers: Array<Signer>
  let swap: SwapV2
  let swapClone: SwapV2
  let swapDeployer: SwapDeployerV2
  let DAI: GenericERC20
  let USDC: GenericERC20
  let USDT: GenericERC20
  let SUSD: GenericERC20
  let swapToken: LPTokenV2
  let owner: Signer
  let user1: Signer
  let user2: Signer
  let attacker: Signer
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      // Deploy Swap Deployer
      swapDeployer = (await (
        await ethers.getContractFactory("SwapDeployerV2")
      ).deploy()) as SwapDeployerV2
      ...
      swapClone = await ethers.getContractAt("SwapV2", swapCloneAddress)
